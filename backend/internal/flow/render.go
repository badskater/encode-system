...
func paramsLiteral(st model.Step, t *model.StepTemplate) string {
	...
	for _, pd := range t.Params {
		...
		parts = append(parts, fmt.Sprintf(" %s = %s", pd.Key, psQuote(val)))
	}
...
// Used by the API before persisting a flow so broken flows never save.
func ValidateForRender(f *model.Flow, resolve TemplateResolver) error {
	if resolve == nil {
		resolve = BuiltinResolver()
	}
	for i, st := range f.Steps {
		if _, err := resolve(st.TemplateKey()); err != nil {
...
// Keep the known-steps registry referenced for future validation use.
var _ = knownSteps
